...
export default class PartyAccessPopup extends Component {
    constructor(props) {
        super(props);
        this.state = {
            visible: props.visible,
            arrAccess: props.arrAccess
        };
    }
            ...
            <Modal
                ...
                <View style={styles.container}>
                    ...
                    <View style={styles.popup}>
                        ...
                        {
                            this.state.arrAccess.map((item, index) => {
                                return <this.renderOptions
                                    item={item}
                                    index={index}
                                    onPress={() => {
                                        let arrAccess = this.state.arrAccess
                                        arrAccess.map((objData, tempIndex) => {
                                            if (tempIndex == index) {
                        ...
                        <View style={{ marginBottom: 20, marginTop: 30 }}>
                            ...
                            <CustomButton
                                text= {translate('Save')}
                                isDisable={this.state.arrAccess.filter((objData) => { return objData.isSelect }).length <= 0}
                                onPress={() => {
                                    let index = this.state.arrAccess.findIndex((objData) => {
                                        return objData.isSelect == true;
                                    });
                                    if (index !== -1) {
                                        this.props.onSave(this.state.arrAccess[index])
                                    }
                                }} />
